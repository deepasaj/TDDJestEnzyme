...
import { useAuthAPI, useIsAdmin } from 'store/store'
...
const Jobs = (props) => {
  ...
  const { workflow_type, job_status } = queryString.parse(history.location.search)

  useEffect(() => {
    const workflowColumnIndex = 6
    if(workflow_type)
      setFilter(filter.map((list, index) => index === workflowColumnIndex ? [workflow_type] : list))
    else
      setFilter(filter.map((list, index) => index === workflowColumnIndex ? [] : list))
  }, [workflow_type, history.location.key])
                ...
                <Input
                  ...
                  endAdornment={
                    <InputAdornment position="end">
                      <Tooltip title={editRow[row] ? "Save Job Name" : "Edit Job Name"}>
                        <span>
                          <IconButton
                            onClick={() => {
    ...
    },
    {
      ...
      options: {
        ...
        customBodyRender: (value, tableMeta) => {
          var jobReady = false;
          if (tableMeta.rowData != undefined && (tableMeta.rowData[5] === user.display_name || isAdmin)) {
            //if the job is yours and status is in one of the below status, can open job wizard
            const workflow_type = tableMeta.rowData[6];
            if ((tableMeta.rowData[4] == "User Input" || tableMeta.rowData[4] == "Ready" || tableMeta.rowData[4] == "Complete") && hasWriteAccess) {
              jobReady = true;
