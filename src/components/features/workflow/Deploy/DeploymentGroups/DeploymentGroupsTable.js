...
const DeploymentGroupsTable = (props) => {
  ...
  const handleOpenDeployments = (tableMeta) => {
    ...
    history.push(`/workflow/deploy/deployment/create/${deployment_id}`);
  };

  //redirect to edit deployment group to edit deployment group
  const handleEditDeployments = (tableMeta) => {
    var deployment_id = tableMeta.rowData[0];
    history.push(`/workflow/deploy/group/edit/${deployment_id}`);
  };
...
//            }, 5000);
//
//            authAPI.fetch(`/workflow/get_back_ref/deploy/id:${group_id}`, {
//                method: 'DELETE'
//            }).then((response) => {
//                clearTimeout(timeout);
//                if (!didTimeOut) {
//                    resolve(response);
//                    window.location.href = `${API_URL}/workflow/deploy/deployment_groups`;
//                }
//            }).catch(error => {
