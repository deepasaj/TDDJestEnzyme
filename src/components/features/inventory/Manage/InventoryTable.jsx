...
const InventoryTable = () => {
  ...
  const deleteMult = () => {
    for (var idx in selectedDevices) {
      authAPI.delete(`/inventory/device/${selectedDevices[idx].id}/delete`)
        .then(() => {
          refreshDevices();
        }).catch(() => {
  ...
  const submitInv = (user_id) => {
    ...
    authAPI.post("/inventory/validate", device_array)
      .then((data) => {
  ...
  const checkStatus = async (job_id, postData) => {
    progress();
    if (showButton === false) {
      let existingDevice;
      ...
      if(existingDevice) {
        ...
        else if (postData.mgmt_ip === existingDevice.mgmt_ip) {
        ...
        else if (postData.hostname === existingDevice.hostname) {
      ...
      setTimeout(() => {
        if (duplicateIP === false && duplicateHost === false) {
          authAPI.get(`/inventory/validation_status/${job_id}`)
            .then(({ data }) => {
              ...
              if (data.code == 102) {
                ...
                if (showButton === false) {
                  var postMsg = JSON.parse(data.data);
      ...
      authAPI.get(`/inventory/validation_status/${job_id}`)
        .then((data) => {
          ...
          if (data.data.code == 102) {
            ...
            //If adding a new device, post, else patch the data
            //Make decision based on what button is shown
            authAPI.patch(`/inventory/device/${id}/update`, { "data": patch_device })
              .then(() => {
